...
function patchEvents(lastEvents, nextEvents, dom) {
    ...
    if (nextEvents !== utils_1.EMPTY_OBJ) {
        for (var name_1 in nextEvents) {
            // do not add a hasOwnProperty check here, it affects performance
            patchEvent(name_1, lastEvents[name_1], nextEvents[name_1], dom);
        }
    }
    if (lastEvents !== utils_1.EMPTY_OBJ) {
        for (var name_2 in lastEvents) {
            // do not add a hasOwnProperty check here, it affects performance
            if (inferno_shared_1.isNullOrUndef(nextEvents[name_2])) {
                patchEvent(name_2, lastEvents[name_2], null, dom);
            }
        }
    }
}
